...
func _ready():
	if is_network_master():
		...
		gui_SendChatBt.connect("pressed",gui_SendChatBt,"printMsg",[$plName.text])
		gui_Info = get_node("/root/world/wcl/Btm_GUI/HB/Info")
		gui_Info.connect("toggled", gui_Info, "_on_Info_toggled")
		$Camera2D.make_current()

	$AnimTr.active = true

func char_In_Focus_State(isPressed):
	qui_Char_In_Focus = isPressed
